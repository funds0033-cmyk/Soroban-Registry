...
export default function ContractTimeline({
  ...
  const [typeFilter, setTypeFilter] = useState<AnalyticsEventType | "all">(
    "all",
  );
  ...
  const handleExport = () => {
    const csvContent =
      ...
      filteredEvents
        .map(
          (e) =>
            `${e.created_at},${e.event_type},${e.user_address || ""},${e.network || ""},"${JSON.stringify(e.metadata).replace(/"/g, '""')}"`,
        )
        .join("\n");
...
>>>>>>> main
        ...
        <div className="relative pl-8 space-y-8 before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-0.5 before:bg-gradient-to-b before:from-primary/50 before:to-transparent">
          {filteredEvents.map((event, index) => (
            <div key={event.id} className="relative group">
              <div
                className={`absolute -left-8 top-1 w-6 h-6 rounded-full border-4 border-background shadow-sm flex items-center justify-center z-10 transition-transform group-hover:scale-110 ${index === 0 ? "bg-primary" : "bg-muted-foreground/20"}`}
              >
                <div
                  className={`w-1.5 h-1.5 rounded-full ${index === 0 ? "bg-primary-foreground" : "bg-muted-foreground"}`}
                ></div>
              </div>

              <div
                className="bg-card hover:bg-accent/30 p-4 rounded-xl border border-border shadow-sm transition-all hover:shadow-md cursor-pointer"
                    ...
                    <div className="flex items-center gap-3 text-xs text-muted-foreground">
                      ...
                      {event.user_address && (
                        <span className="flex items-center gap-1 font-mono">
                          ...
                          {event.user_address.slice(0, 4)}...
                          {event.user_address.slice(-4)}
                        </span>
                      )}
                    </div>
                  </div>
      ...
      {selectedEvent && (
        ...
        >
          <div
            className="bg-card w-full max-w-2xl rounded-2xl shadow-2xl border border-border overflow-hidden animate-in fade-in zoom-in duration-200"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="p-6 border-b border-border flex items-center justify-between bg-muted/30">
              <div className="flex items-center gap-3">
                <div className="p-2 rounded-xl bg-background border border-border shadow-inner">
                  {eventIcons[selectedEvent.event_type] || (
            ...
            <div className="p-6 space-y-6 max-height-[70vh] overflow-y-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1 bg-muted/20 p-3 rounded-lg border border-border">
                  ...
                  <p className="font-mono text-sm break-all">
                    {selectedEvent.user_address || "System"}
                  </p>
                </div>
              </div>
