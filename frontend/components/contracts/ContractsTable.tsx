    ...
    >
      <div className="flex items-center gap-1.5 break-keep whitespace-nowrap">
        ...
        <div className="flex flex-col opacity-50 ml-1">
          <ArrowUp
            className={`w-3 h-3 ${header.column.getIsSorted() === "asc" ? "opacity-100 text-primary" : "opacity-30"}`}
          />
          <ArrowDown
            className={`w-3 h-3 -mt-1 ${header.column.getIsSorted() === "desc" ? "opacity-100 text-primary" : "opacity-30"}`}
          />
        </div>
      </div>
    </th>
        ...
        cell: ({ row }) => (
          <div className="max-w-[180px]">
            ...
            <div className="text-[10px] text-muted-foreground font-mono truncate">
              {row.original.contract_id.substring(0, 10)}...
            </div>
          </div>
        ),
      },
...
>>>>>>> main
  ...
  const table = useReactTable({
    ...
    onSortingChange: (updater) => {
      const computedState =
        typeof updater === "function" ? updater(sortingWrapper) : updater;
      if (computedState.length > 0) {
        onSortChange(
          reverseSortByContext(computedState[0].id),
          computedState[0].desc ? "desc" : "asc",
        );
      }
    },
                ...
                {table.getRowModel().rows.map((row) => (
                  ...
                  >
                    {row.getVisibleCells().map((cell) => (
                      <td
                        key={cell.id}
                        className="px-4 py-3 align-middle bg-transparent break-keep"
                      >
                        {flexRender(
