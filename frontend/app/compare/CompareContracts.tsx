        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr className="border-b border-border">
              ...
              {contracts.map((c) => (
                <th
                  key={c.id}
                  className="py-2 px-3 text-left text-xs font-semibold text-foreground min-w-[160px]"
                >
                  {c.name}
                </th>
              ))}
          ...
          <tbody>
            ...
            <tr className="border-b border-border/50">
              ...
              {contracts.map((c) => (
                <td key={c.id} className="py-2 px-3 text-xs text-foreground">
                  {c.versionCount}
                </td>
              ))}
            </tr>
    ...
    return (
      <div className="rounded-2xl border border-border bg-card p-5">
        ...
        <p className="mt-3 text-xs text-muted-foreground">
          All selected contracts share the same ABI methods — no unique methods
          found.
        </p>
      </div>
    );
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        Methods present in one contract but absent from all others.
      </p>
...
export default function CompareContracts() {
  ...
  const exportRowsForPdf = useMemo(
    () => [
      ...
      {
        label: "Tags",
        values: selectedContracts.map((c) =>
          c.tags.length > 0 ? c.tags.join(", ") : "None",
        ),
      },
    ],
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Select 2-4 contracts, compare real contract details
                side-by-side, and inspect ABI and source differences.
              </p>
            </div>
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  disabled={!canExport}
                  onClick={() =>
                    exportComparisonToPdf(selectedContracts, exportRowsForPdf)
                  }
                  className={`inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-sm font-semibold transition-colors ${
                    canExport
                      ? "border-border bg-background text-muted-foreground hover:bg-accent hover:text-foreground"
                ...
                <button
                  ...
                  onClick={() =>
                    copy(window.location.href, {
                      successEventName: "comparison_link_copied",
                    })
                  }
                  disabled={isCopying}
        ...
        {viewMode === "table" ? (
          <div className="flex flex-col gap-4">
            {!selectionCountValid && (
              <div className="rounded-2xl border border-border bg-card p-6">
                <div className="text-sm font-semibold text-foreground">
                  Add at least 2 contracts to compare.
                </div>
                <div className="mt-1 text-xs text-muted-foreground">
                  Use the selector above to pick contracts.
                </div>
              </div>
            )}
                  ...
                  <div className="rounded-2xl border border-border bg-card p-5">
                    ...
                    <div className="mt-3 space-y-2">
                      {selectedContracts.map((contract) => (
                        <div
                          key={contract.id}
                          className="flex items-center justify-between gap-3 rounded-xl bg-accent/40 px-3 py-2 text-sm"
                        >
                      ...
                      {selectedContracts.map((contract) => (
                        ...
                        >
                          ...
                          <span className="shrink-0 font-semibold text-foreground">
                            {contract.abiMethods.length} methods
                          </span>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="rounded-2xl border border-border bg-card p-5">
                    ...
                    <div className="mt-3 space-y-2">
                      {selectedContracts.map((contract) => (
                        <div
                          key={contract.id}
                          className="rounded-xl bg-accent/40 px-3 py-2 text-sm"
                        >
